...
if columna_edad not in df_normal.columns:
    raise ValueError(f"La columna '{columna_edad}' no se encontró en el dataset.")

# Convertir la columna de edad a numérica
df_normal[columna_edad] = pd.to_numeric(df_normal[columna_edad], errors="coerce")

# Eliminar filas con edades nulas o fuera de rango
df_normal = df_normal.dropna(subset=[columna_edad])
df_normal = df_normal[df_normal[columna_edad] >= 50]  # Filtrar edades menores a 50 años

bins = list(range(50, int(df_normal[columna_edad].max()) + 6, 5))
labels = [f"{bins[i]}-{bins[i+1]-1}" for i in range(len(bins)-1)]

# Crear la nueva columna de rangos de edad
df_normal["Rango de Edad"] = pd.cut(df_normal[columna_edad], bins=bins, labels=labels, right=False)

# Contar la cantidad de pacientes en cada rango de edad
conteo_edades = df_normal["Rango de Edad"].value_counts().sort_index()
...
plt.title("Pacientes con VALORACIÓN COGNITIVA NORMAL por Edades")
